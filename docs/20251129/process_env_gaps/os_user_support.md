...
- **Proposed fix:** Thread `options.user` into transport/exec layers (Port, built-in transport, and any custom transport), add validation, and cover with integration tests (ensuring commands run as the requested user when supported by the platform).
