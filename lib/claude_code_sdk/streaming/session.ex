defmodule ClaudeCodeSDK.Streaming.Session do
  ...
  alias ClaudeCodeSDK.Streaming.EventParser
  ...
  ## Parameters
  - `options` - Optional `ClaudeCodeSDK.Options` struct
    ...
    if state.active_subscriber == ref do
      ...
      case :exec.send(pid, json_msg <> "\n") do
        ...
        {:error, reason} ->
          ...
          broadcast_error_to(state.active_subscriber, state.subscribers, reason)
          {:noreply, state}
      end
    else
          ...
          case state.subscriber_queue do
            [{next_ref, next_message} | rest] ->
              ...
              case :exec.send(pid, json_msg <> "\n") do
                ...
                {:error, reason} ->
                  Logger.error("Failed to send queued message: #{inspect(reason)}")
              end
  ...
  defp broadcast_error_to(ref, subscribers, reason) do
    case Map.get(subscribers, ref) do
      nil -> :ok
