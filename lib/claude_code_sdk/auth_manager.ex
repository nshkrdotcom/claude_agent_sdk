defmodule ClaudeCodeSDK.AuthManager do
  ...
  alias ClaudeCodeSDK.Auth.{Provider, TokenStore}
  ...
  def handle_call(:get_token, _from, state) do
    cond do
      env_key_present?() ->
        # Prefer CLAUDE_CODE_OAUTH_TOKEN, fallback to ANTHROPIC_API_KEY
        token = System.get_env("CLAUDE_CODE_OAUTH_TOKEN") || System.get_env("ANTHROPIC_API_KEY")
  ...
  defp env_key_present? do
    # Check both ANTHROPIC_API_KEY and CLAUDE_CODE_OAUTH_TOKEN
    case {System.get_env("ANTHROPIC_API_KEY"), System.get_env("CLAUDE_CODE_OAUTH_TOKEN")} do
      {nil, nil} -> false
      {"", ""} -> false
      {nil, ""} -> false
      {"", nil} -> false
