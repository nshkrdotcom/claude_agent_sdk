defmodule ClaudeAgentSDK.OptionBuilder do
  @moduledoc """
  ...
  **Features**:
  ...
  - `effort` - Effort level (`:low`, `:medium`, `:high`, `:xhigh`, or `:max`). Invalid values raise `ArgumentError`. `:xhigh` is Opus-only. Not supported for Haiku.
  """
  @spec with_effort(Options.t(), :low | :medium | :high | :xhigh | :max) :: Options.t()
  def with_effort(%Options{} = options, effort)
      when effort in [:low, :medium, :high, :xhigh, :max] do
    %{options | effort: effort}
  end

  def with_effort(%Options{}, effort) do
    raise ArgumentError,
          "effort must be one of :low, :medium, :high, :xhigh, or :max, got: #{inspect(effort)}"
  end
